...
1. 打开右上角 **设置 → 媒体库管理 → 添加文件夹**
2. 点「添加文件夹」后会直接弹出文件夹选择器，逐层选中目标文件夹
3. 选类型：**视频库** 或 **图片库**（创建后不可更改）
4. 分类可留空；填了就会成为列表页顶部的筛选胶囊，例如「电影」「剧集」
5. 保存后到 **设置 → 扫描与更新**，点 **开始扫描**
...
播放页不会硬播，而是给出两条出路：尝试唤起系统默认播放器，或复制路径手动打开。
...
`POST /api/scan/` 触发，同一时刻只允许一个扫描任务。
