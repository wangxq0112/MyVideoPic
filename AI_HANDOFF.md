...
| Scanner | `backend/videos/scanner.py`, `scanning/` | Background incremental scans, bounded photo preparation, and task progress |
...
- New or changed photos are decoded, EXIF-read, and thumbnail-generated by at
  most two worker threads. The scan thread remains the only SQLite writer;
  do not move ORM writes into photo workers.
